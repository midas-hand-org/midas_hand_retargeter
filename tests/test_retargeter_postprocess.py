...
from midas_hand_retargeter.retargeter import (
    _finger_targets_from_landmarks,
    _thumb_targets_from_landmarks,
)
...
def test_finger_postprocess_curls_mcp_and_pip():
    open_targets = _finger_targets_from_landmarks(_straight_landmarks())
    curled_targets = _finger_targets_from_landmarks(_curled_landmarks())
...
def test_finger_abduction_responds_to_lateral_splay():
    ...
    targets = _finger_targets_from_landmarks(landmarks)
...
def test_thumb_roll_decreases_with_opposition():
    open_landmarks = _straight_landmarks()
    pinch_landmarks = _straight_landmarks()
    pinch_landmarks[4] = pinch_landmarks[8] + [0.005, 0.0, 0.0]

    open_targets = _thumb_targets_from_landmarks(open_landmarks)
    pinch_targets = _thumb_targets_from_landmarks(pinch_landmarks)

    assert pinch_targets["thumb_cmc_roll_joint"] < open_targets["thumb_cmc_roll_joint"]
    assert pinch_targets["thumb_cmc_side_joint"] > open_targets["thumb_cmc_side_joint"]
